...
namespace nsK2Engine {
	...
	void SpriteRender::CircleInit(float& p,float&size,const char*filePath,const float w,const float h, AlphaBlendMode alphaBlendMode)
	{
		SpriteInitData initData;
		//DDSファイル(画像データ)のファイルパスを指定する。
		initData.m_ddsFilePath[0] = filePath;
		...
		initData.m_expandConstantBuffer = m_circleData;
		initData.m_expandConstantBufferSize = sizeof(CircleData);
